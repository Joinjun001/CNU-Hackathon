# 🚀 Word402 (AgentPress-402)

> **워드프레스용 x402 프로토콜 기반 AI 에이전트 대상 초소액 유료 콘텐츠 게이트웨이 & 자율 결제 시스템**  
> *"AI 에이전트를 차단하지 말고, 초소액으로 판매하라."*

<p align="left">
  <img src="https://img.shields.io/badge/Blockchain-Base%20Sepolia%20(L2)-blue?style=flat-square" alt="Base Sepolia" />
  <img src="https://img.shields.io/badge/Settlement-USDC%20(ERC--20)-green?style=flat-square" alt="USDC" />
  <img src="https://img.shields.io/badge/CMS-WordPress%206.x-21759B?style=flat-square&logo=wordpress" alt="WordPress" />
  <img src="https://img.shields.io/badge/Protocol-x402%20v2.0-orange?style=flat-square" alt="x402 Protocol" />
  <img src="https://img.shields.io/badge/Language-PHP%208.3%20%7C%20Python%203.10+-yellow?style=flat-square" alt="Language" />
  <img src="https://img.shields.io/badge/Hackathon-BLOCK%20AI%20%2A%2026-blueviolet?style=flat-square" alt="COSS 2026" />
</p>

* **대회명**: Web3 시대의 블록체인 AI 융합 해커톤
* **참가 트랙**: **트랙 02. 블록체인 + 데이터 마켓플레이스**
* **배포 사이트**: [https://injun-cloud.duckdns.org](https://injun-cloud.duckdns.org)
* **공개 온체인 원장 (Live Ledger)**: [https://injun-cloud.duckdns.org/ledger/](https://injun-cloud.duckdns.org/ledger/)
* **시연 영상 (YouTube Demo)**: [https://www.youtube.com/watch?v=Cwa3BDvchQc](https://www.youtube.com/watch?v=Cwa3BDvchQc)

---

## 📌 1. 🚨 문제 정의 및 개발 배경 (Why Word402?)

### 1.1. 웹 콘텐츠 생태계의 30년 수익 공식 붕괴
지난 30년간 블로그, 언론사, 웹 미디어를 지탱해 온 핵심 비즈니스 모델은 **"검색 노출 ➔ 인간 유저 방문 ➔ 배너 광고 노출 및 리퍼럴(Referral) 클릭 수익"**이었습니다. 웹 생태계의 모든 경제학은 오직 '인간의 시선과 체류 시간'을 전제로 설계되어 있었습니다.

### 1.2. AI 에이전트 트래픽의 급증과 수익화의 구조적 단절
그러나 최근 Perplexity, ChatGPT Search, Claude 및 수많은 자율형 AI 에이전트가 등장하면서 웹 트래픽의 주체가 **인간에서 AI로 급격히 전환**되고 있습니다.
* **광고 모델의 무력화**: AI 에이전트는 인간처럼 배너 광고를 보거나 제휴 링크를 클릭하지 않고, 웹 콘텐츠의 핵심 데이터만 즉시 요약·소비합니다.
* **비용과 편익의 불균형**: 사이트 운영자는 AI 크롤러의 잦은 접근으로 인해 서버 트래픽 비용을 그대로 부담하지만, 정작 데이터에 대한 광고 수익은 증발하는 구조적 위기에 직면했습니다.

### 1.3. 기존 대응 방식(단순 차단)의 한계와 트래픽 낭비
현재 대다수 웹 퍼블리셔는 `robots.txt` 설정이나 WAF(방화벽)를 통해 AI 봇을 차단하는 대응을 취하고 있습니다.
* 하지만 무조건적인 차단은 AI 생태계에서의 데이터 노출 기회를 스스로 박탈할 뿐만 아니라, 우회 크롤러와의 소모적인 '차단-우회 전쟁'으로 양측 모두에게 트래픽과 인프라 비용만 낭비시키는 결과를 초래했습니다.
* AI 기업 입장에서도 합법적이고 신뢰할 수 있는 고품질 데이터를 안정적으로 수급할 수 있는 통로가 막히게 되었습니다.

### 1.4. 해결책: x402 기반 자율 정산 프로토콜 구현
이에 본 팀은 **"AI를 차단하지 말고, 머신이 지불 가능한 초소액으로 합당한 대가를 받고 판매하자"**는 패러다임 전환을 시도했습니다.
* 1990년대 웹 표준으로 제정되었으나 결제 인프라 부재로 잠들어 있던 **`HTTP 402 Payment Required`** 상태 코드를 최신 Web3 L2 블록체인(Base Sepolia)과 결합했습니다.
* AI 에이전트가 데이터에 접근할 때 402 페이월을 반환하고, 에이전트가 자체 지갑으로 초소액(USDC)을 자율 결제하면, 광고 노이즈가 제거된 **LLM 최적화 정제 Markdown 데이터**를 즉시 공급하는 M2M 데이터 마켓플레이스 `Word402`를 구현했습니다.

---

## 🔄 2. 시스템 아키텍처 및 x402 프로토콜 시퀀스

Word402는 웹 서버와 AI 에이전트 간의 복잡한 회원가입이나 PG 결제창 없이, **순수한 HTTP Header/JSON 교환과 스마트 온체인 트랜잭션만으로 결제와 데이터 제공을 완결**합니다.

```mermaid
sequenceDiagram
    autonumber
    actor AI as 🤖 자율 AI 에이전트
    participant WP as 🌐 WordPress (Word402 플러그인)
    participant RPC as ⛓️ Base Sepolia (EVM L2)

    AI->>WP: GET /wp-json/word402/v1/posts/8 (Accept: text/markdown)
    WP-->>AI: HTTP 402 Payment Required
    Note over WP,AI: 수취 지갑, 단가(0.005 USDC), 체인ID, 챌린지 Nonce 반환

    Note over AI: 에이전트 자체 지갑(Private Key)으로<br/>Base Sepolia USDC 전송 서명
    AI->>RPC: eth_sendRawTransaction (0.005 USDC Transfer)
    RPC-->>AI: 블록 포함 확정 (Tx Hash 생성)

    AI->>WP: GET /wp-json/word402/v1/posts/8<br/>Header: X-Payment: {tx_hash, challenge_id}
    
    WP->>RPC: eth_getTransactionReceipt (온체인 영수증 조회)
    RPC-->>WP: 트랜잭션 성공 여부, 수취인, 입금액, 블록번호 반환
    
    Note over WP: 1) 온체인 입금 사실 검증<br/>2) Nonce & 영수증 재사용 방지(Anti-Replay) 확인<br/>3) 원장(Ledger) 기록

    WP-->>AI: HTTP 200 OK (정제된 Clean Markdown 본문 서빙)
    Note over AI: 획득한 본문 기반 실시간 AI 요약 브리핑 생성
```

---

## 🌐 3. 실시간 온체인 실증 증거 (Live Verification Proof)

Word402는 단순 목업이나 이론적 설계에 그치지 않고, **실제 Base Sepolia L2 블록체인 상에서 실제 USDC 전송 및 실시간 RPC 검증을 100% 성공적으로 완결**했습니다.

* **🎬 시연 영상 (YouTube Demo)**: [https://www.youtube.com/watch?v=Cwa3BDvchQc](https://www.youtube.com/watch?v=Cwa3BDvchQc)
* **실시간 온체인 TX HASH**: [`0x719f3cd4ed73d0237825cde2c3d76b87f27217df72c8a1e47f553e4fab549a9f`](https://sepolia.basescan.org/tx/0x719f3cd4ed73d0237825cde2c3d76b87f27217df72c8a1e47f553e4fab549a9f) (Block #46761373)
* **결제 정산 내역**: `0.005 USDC` (구매자: `0xb045FD...b0D999` ➔ 판매자: `0xf49FA4...784FdD`)

| ① 실시간 온체인 결제 원장 ([/ledger/](https://injun-cloud.duckdns.org/ledger/)) | ② 고가치 데이터 포털 메인 ([홈페이지](https://injun-cloud.duckdns.org)) |
| :---: | :---: |
| ![Live Ledger](docs/images/ledger_dashboard.png) | ![Portal Main](docs/images/portal_main.png) |
| *Base Sepolia 온체인 영수증 실시간 동기화 대시보드* | *AI 에이전트 전용 고품질 지식 데이터 포털* |

---

## 🛠️ 4. 5대 핵심 기술 모듈 (Technical Depth)

| 핵심 모듈 | 역할 및 구현 내용 | 기술적 특징 |
| :--- | :--- | :--- |
| **지능형 트래픽 인터셉터** | 인간 방문자는 무료로 글을 열람하고, AI 에이전트 특성 감지 시에만 선별적 402 반환 | 기존 웹 UX 보존, `Accept` / User-Agent 정밀 분기 |
| **온체인 실시간 검증 엔진** | 에이전트가 제출한 Tx Hash를 Base Sepolia 공식 RPC 노드로 즉시 질의 | `eth_getTransactionReceipt` 원자적(Atomic) 영수증 대조 |
| **재사용 방지 원장 가드** | 일회성 Nonce와 Tx Hash를 DB에 등록하여 영수증 재사용 공격(Replay Attack) 방지 | `wp_x402_receipts` 기반, 중복 제출 시 `409 Conflict` 차단 |
| **Clean Markdown 정제기** | 결제 승인 즉시 광고, 사이드바 등 불필요한 HTML 노이즈를 제거한 순수 본문 공급 | LLM 토큰 소모 최소화 및 RAG 데이터 공급 최적화 |
| **자율 결제 에이전트 SDK** | 402 감지 ➔ 잔액 조회 ➔ 온체인 서명/송금 ➔ 데이터 수신 및 요약 전 과정 무인화 | Python `web3.py` 기반의 완전 자율 결제 클라이언트 |

---

## 💼 5. 비즈니스 모델 및 시장 파급력 (BM & Practicality)

### 5.1. 왜 워드프레스(WordPress)인가? (Go-to-Market 전략)
전 세계 웹사이트의 **43.5%(약 8억 1천만 개 사이트)**가 워드프레스로 구동됩니다. Word402는 기존 퍼블리셔에게 **"플러그인 설치 한 번"**으로 즉시 AI 대상 데이터 판매를 활성화할 수 있는 가장 강력한 초기 시장 침투력(Beachhead)을 가집니다.

### 5.2. 상생형 비즈니스 모델 (Win-Win Economics)
1. **콘텐츠 생산자 (퍼블리셔)**: 검색 유입 감소로 사라진 광고 수익을 AI 대상 건당 0.005 USDC 마이크로 수익으로 대체.
2. **AI 에이전트 (소비자)**: 거대 언론사와의 값비싼 일괄 독점 계약 없이, 필요한 순간 필요한 지식만 초소액으로 합법적 취득.
3. **Word402 플랫폼 수익**: 결제 발생 시 1% 프로토콜 트레저리 적립 및 엔터프라이즈 미디어용 분석 대시보드 라이선스 공급.

---

## 🚀 6. 향후 확장 로드맵 및 기술적 개선 방향 (Roadmap & Evolution)

Word402의 비전은 단순한 CMS 플러그인을 넘어, **AI 에이전트가 디지털 세계의 모든 자원을 무인으로 거래하는 '탈중앙화 M2M 인프라 결제 레일'**로 확장되는 것입니다.

### 6.1. 클라우드 컴퓨팅 서비스 자원(M2M Compute Gateway)으로의 확장
* **서버리스 GPU / AI 추론 연산 (Pay-per-Inference)**: AI 에이전트가 고성능 추론 모델(DeepSeek, Llama-3 70B 등)이 필요할 때, 신용카드 없이 초/토큰 단위(건당 0.001 USDC)로 402 결제 후 즉시 GPU 컨테이너 할당.
* **범용 클라우드 API 리버스 프록시 (Envoy / Cloudflare Worker)**: 기존 날씨, 금융, 퀀트 API 앞단에 배치하여 코드 수정 없이 'AI 전용 유료 종량제 API'로 즉시 전환.

### 6.2. 보안성 강화 및 차세대 아키텍처 고도화 과제
* **EIP-712 상호 전자서명 도입**: 멤풀 Tx Hash 탈취(Sniffing) 공격을 방지하기 위해 헤더에 `X-Agent-Signature`를 요구하고 서버가 `ecrecover`로 송금 지갑과 요청자의 일치성을 암호학적으로 검증.
* **오프체인 바우처 세션 (State Channel)**: 스마트 컨트랙트 예치금을 기반으로 수천 회의 고빈도 API 호출을 가스비 없이 ms 단위의 EIP-712 오프체인 서명 바우처로 처리 후 최종 차액만 일괄 정산.
* **가스비 없는(Gasless) 결제 UX (ERC-4337 & EIP-3009 Paymaster)**: USDC 위임 서명 및 계정 추상화 페이마스터를 연동하여, 에이전트가 가스비(ETH) 없이 오직 USDC만으로 모든 결제를 완결.

---

## 📂 7. 프로젝트 디렉터리 구조

```text
CNU-Hackathon/
├── README.md                      # 프로젝트 공식 백서 및 실증 문서 (본 파일)
├── .env.example                  # 환경 변수 템플릿 (Base Sepolia RPC, 컨트랙트 주소 등)
├── docker-compose.yml            # 워드프레스 + MySQL 로컬/서버 컨테이너 정의
├── agent-client/                 # AI 에이전트 클라이언트 (자율 402 감지 및 온체인 결제)
├── word402/                      # 워드프레스 플러그인 (인터셉터, 온체인 검증기, 공개원장)
├── docs/                         # 아키텍처 다이어그램, ERD, 프로토콜 상세 기술 문서
└── tests/                        # 단위 테스트 및 핸드셰이크 검증 코드
```

---



## 📚 8. 참고자료 및 레퍼런스 (References & External Links)

본 프로젝트 연구 및 개발 과정에서 참고한 레퍼런스 목록입니다:

* **x402 프로토콜 공식 규격**: [https://x402.org/](https://x402.org/) (Coinbase 파트너십 x402 표준 Specification)
* **x402 온체인 익스플로러**: [https://www.x402scan.com/](https://www.x402scan.com/) (x402 프로토콜 트랜잭션 스캔 탐색기)
* **Micro Payment Protocol (MPP)**: [https://mpp.dev/](https://mpp.dev/) (마이크로 페이먼트 프로토콜 개발자 문서)
* **MPP Scan**: [https://mppscan.com/](https://mppscan.com/) (MPP 익스플로러)
* **Cloudflare Monetization Gateway**: [https://blog.cloudflare.com/monetization-gateway/](https://blog.cloudflare.com/monetization-gateway/) (Cloudflare AI 콘텐츠 수익화 블로그)
* **웹 크롤링 통계 & 벤치마크**: [https://thunderbit.com/ko/blog/web-crawling-stats-and-industry-benchmarks](https://thunderbit.com/ko/blog/web-crawling-stats-and-industry-benchmarks) (웹 데이터 크롤링 트래픽 벤치마크 리포트)
* **AI 데이터 경제 & 유료화 동향**: [https://www.techflowpost.com/ko/article/33882](https://www.techflowpost.com/ko/article/33882) (AI 에이전트 시대 데이터 마켓 아티클)



